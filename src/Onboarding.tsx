    ...
    <Stack as="form" onSubmit={onSubmt} spacing="4">
      <FormControl id="userName" size="lg" color="teal.400">
        ...
        <Input
          ...
          onChange={(e) => dispatch({ type: "updateUser", payload: e.target.value })}
          value={userName} />
      </FormControl>
    ...
    <Stack as="form" onSubmit={onSubmit} spacing="4">
      <FormControl id="companyName" size="lg" color="teal.400">
        ...
        <Input
          ...
          onChange={(e) => dispatch({ type: "updateCompany", payload: e.target.value })}
          value={companyName} />
      </FormControl>
...
export function ShareholdersStep() {
  ...
  return (
    <Stack>
          ...
          <ModalContent>
            <Stack p="10" as="form" onSubmit={submitNewShareholder}>
              ...
              <Select
                ...
                onChange={(e) => setNewShareholder((s) => ({
                  ...s,
      ...
      <Button as={Link} to="../grants" colorScheme="teal">
        Next
      </Button>
    </Stack>
  );
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalContent>
          <Stack p="10" as="form" onSubmit={submitGrant}>
            ...
            <FormControl>
              <Select
                placeholder="Stock Preference"
                value={draftGrant.type}
                data-testid="grant-type"
                onChange={(e) =>
                  // TODO: bad any
                  setDraftGrant((g) => ({ ...g, type: e.target.value as any }))}
              >
...
export function DoneStep() {
  ...
  const navigate = useNavigate();
  const { email, userName, companyName, shareholders, grants } = useContext(OnboardingContext);

  const grantMutation = useMutation<Grant, unknown, Grant>((grant) => fetch("/grant/new", {
    method: "POST",
  ...
  const shareholderMutation = useMutation<Shareholder, unknown, Shareholder>(
    (shareholder) => fetch("/shareholder/new", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const userMutation = useMutation<User, unknown, User>((user) => fetch("/user/new", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(user),
  }).then((res) => res.json())
  );
  const companyMutation = useMutation<Company, unknown, Company>((company) => fetch("/company/new", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
type OnboardingAction = UpdateUserAction |
  UpdateEmail |
  UpdateCompanyAction |
  ...
  return produce(state, (draft) => {
    switch (action.type) {
      ...
      case "updateCompany":
        ...
        break;
      case "addShareholder":
        const nextShareholderID = Math.max(
          0,
  ...
  return (
    <OnboardingContext.Provider value={{ ...state, dispatch }}>
      <Stack direction="column" alignItems="center" spacing="10">
        ...
        {/* TODO: Don't allow navigation to later steps with precursor data missing */}
        <Routes>
